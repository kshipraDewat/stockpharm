...
export default function DoctorLoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await api.post('/accounts/doctor/login', { email, password });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-emerald-50 p-6">
      <form onSubmit={submit} className="w-full max-w-md bg-white rounded-2xl p-8 shadow-lg space-y-3">
        ...
        <p className="text-sm text-center"><Link to="/doctor/register" className="text-emerald-600">Register as doctor</Link></p>
      </form>
    </div>
  );
}
