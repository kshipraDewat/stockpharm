...
export default function DoctorRegisterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const r = await api.post('/accounts/doctor/register', form);
      setUser({ ...r.data.user, accountKind: 'doctor', tenantType: 'doctor' });
      toast.success('Registration submitted — pending platform approval');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-emerald-50 p-6">
      <form onSubmit={submit} className="w-full max-w-md bg-white rounded-2xl p-8 shadow-lg space-y-3">
        ...
        {(['name', 'email', 'phone', 'specialization', 'password'] as const).map((f) => (
        ...
        <p className="text-sm text-center"><Link to="/doctor/login" className="text-emerald-600">Sign in</Link></p>
      </form>
    </div>
  );
}
